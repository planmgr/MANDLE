...
export default async function AdminStylePage() {
  const articles = await getStyleArticles(undefined, 100);
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border-light bg-surface-card">
                ...
                <th className="px-4 py-3 font-caption text-[10px] font-medium tracking-[1.5px] text-fg-secondary">FEATURED</th>
              ...
              {articles.map((article) => (
                <tr key={article.id} className="border-b border-border-light last:border-0 hover:bg-surface-card/50 transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/admin/style/${article.id}`} className="font-body text-[13px] text-fg-primary hover:underline">
                      {article.title}
                    </Link>
                  </td>
                  <td className="px-4 py-3 font-caption text-[10px] tracking-[1.5px] text-fg-secondary">{article.category}</td>
                  <td className="px-4 py-3 font-caption text-[11px] text-fg-secondary">{article.is_featured ? "Yes" : "—"}</td>
