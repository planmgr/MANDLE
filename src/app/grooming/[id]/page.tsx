...
import { getGroomingArticleById, getGroomingArticles } from "@/lib/queries/grooming";
import GroomingArticleCard from "@/components/grooming/GroomingArticleCard";
import MarkdownRenderer from "@/components/ui/MarkdownRenderer";

interface GroomingDetailPageProps {
  params: Promise<{ id: string }>;
}

export async function generateMetadata({ params }: GroomingDetailPageProps): Promise<Metadata> {
  const { id } = await params;
  const article = await getGroomingArticleById(Number(id));
  if (!article) return { title: "GROOMING — MANDLE" };
...
export default async function GroomingDetailPage({ params }: GroomingDetailPageProps) {
  const { id } = await params;
  ...
  const relatedArticles = (await getGroomingArticles(article.category, 4)).filter(
    (a) => a.id !== article.id
  );
    ...
    <main>
      ...
      <section className="section-px py-8 md:py-12">
        ...
        <div className="mt-10 pt-8 border-t border-border-light max-w-[960px] mx-auto">
          <Link
            href={`/grooming?category=${article.category}`}
        ...
        <section className="section-px py-9 md:py-12 bg-surface-card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6">
            {relatedArticles.slice(0, 3).map((a) => (
              <GroomingArticleCard key={a.id} article={a} />
            ))}
          </div>
        </section>
      )}
