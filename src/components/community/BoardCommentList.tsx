...
import { addBoardComment, deleteBoardComment } from "@/lib/actions/community";
import { timeAgo } from "@/lib/utils/tags";
import type { BoardComment } from "@/lib/types/community";
...
export default function BoardCommentList({ boardPostId, initialComments, currentUserId }: BoardCommentListProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const newComment = await addBoardComment(boardPostId, content.trim());
      setComments([...comments, newComment as BoardComment]);
      setContent("");
    } catch {
      alert("댓글 작성에 실패했습니다.");
    }
